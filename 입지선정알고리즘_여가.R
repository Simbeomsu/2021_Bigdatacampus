...
### 여가 송중동 용신동 제기동 송천동 전농1동
##송중동
# 데이터 불러오기
options(digits=10)
dong = "송중동"

metro = fread('여가지하철위치3.csv')
bus = fread("여가버정위치.csv")
welfare = fread("기존복지시설.csv")
building = fread("여가건물3.csv")
...
first_lat = f_building_lat[2998]
first_lon = f_building_lon[2998]


##선정지역 근방 300미터 건물 제외 후 적합
...
second_lat = f2_building_lat[1570]
second_lon = f2_building_lon[1570]
...
first_lat = f_building_lat[2597]
first_lon = f_building_lon[2597]
...
second_lat = f2_building_lat[2072]
second_lon = f2_building_lon[2072]
